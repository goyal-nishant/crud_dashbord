        ...
        ?>
                <div class="outer-comment-container">
                    ...
                    while ($comment = mysqli_fetch_assoc($comments_result)) {
                        echo "<div id='comment-{$comment['id']}' class='comment' style='margin-left: " . ($level * 20) . "px;background-color:white;padding:30px'>";
                        echo "<p class='container custom_data' style='height:auto;'><strong>{$comment['commenter_name']}</strong>: {$comment['comment_text']} <br>at: {$comment['created_at']}</p>";
                        if ($replyers_name === $comment['commenter_name']) {
                            echo "<div class='comment-actions'>";
                            echo "<button class='delete-comment-button btn btn-danger'  data-comment-id='{$comment['id']}'>Delete</button>";
                            echo "</div>";
                        }
                        ...
                        echo "<input type='submit' name='submit_reply' id='btnn' value='Submit Reply'>";
                        echo "</form>";
